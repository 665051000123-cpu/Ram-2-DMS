...
export default function DepartmentSettings({ 
  ...
  const [confirmModal, setConfirmModal] = useState<{isOpen: boolean, deptId: string, deptName: string}>({
    isOpen: false, deptId: '', deptName: ''
  });
    ...
    <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
      ...
      <div className="p-6 border-b border-slate-200 flex items-center justify-between bg-slate-50/50">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-lg font-bold text-slate-800">จัดการรายชื่อแผนก</h2>
        </div>
        <div className="flex gap-3">
          ...
          <div className="relative">
            ...
            <input
              ...
              className="block w-full md:w-64 pl-9 pr-4 py-2 bg-white border border-slate-300 rounded-xl text-slate-700 focus:outline-none focus:ring-2 focus:ring-blue-500/50 focus:border-blue-500 transition-all text-sm"
            />
          </div>
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="bg-white text-slate-500 text-sm border-b border-slate-200">
              ...
              <th className="font-semibold py-4 px-6 text-center">จำนวนผู้ใช้งาน</th>
              <th className="font-semibold py-4 px-6 text-center">จำนวนเอกสาร</th>
          ...
          <tbody className="divide-y divide-slate-100">
            {filteredDepartments.length > 0 ? (
              ...
              <tr key={dept.id} className="hover:bg-slate-50 transition-colors">
                ...
                <td className="py-4 px-6 text-center">
                  <div className="inline-flex items-center gap-1.5 px-3 py-1 bg-blue-50 text-blue-600 rounded-full text-sm font-medium">
                    <FileText size={14} />
                ...
                <td className="py-4 px-6">
                  <div className="flex items-center justify-center">
                    <button
                      ...
                      disabled={dept._count.users > 0 || dept._count.documents > 0}
                      className="p-2 text-slate-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition disabled:opacity-30 disabled:hover:text-slate-400 disabled:hover:bg-transparent"
                      title={dept._count.users > 0 || dept._count.documents > 0 ? "ไม่สามารถลบแผนกที่มีข้อมูลอยู่ได้" : "ลบแผนก"}
                    >
                      <Trash2 size={18} />
                    </button>
                  </div>
                ...
                <td colSpan={5} className="py-12 text-center">
                  <div className="inline-flex flex-col items-center justify-center text-slate-400">
                    ...
                    <p className="text-base font-medium text-slate-500">
                      {departments.length === 0 ? 'ยังไม่มีข้อมูลแผนกในระบบ' : 'ไม่พบแผนกที่ค้นหา'}
                    </p>
                  </div>
                </td>
              </tr>
      ...
      <ConfirmModal 
        ...
        onCancel={() => setConfirmModal({ isOpen: false, deptId: '', deptName: '' })}
      />
    </div>
  );
}
