...
export async function DELETE(
  ...
  try {
    ...
    if (userToDelete.departmentId !== session.user.departmentId && session.user.role !== 'SUPER_ADMIN') {
...
export async function PUT(
  ...
  try {
    ...
    if (userToUpdate.departmentId !== session.user.departmentId && session.user.role !== 'SUPER_ADMIN') {
    ...
    if (email !== userToUpdate.email) {
      ...
      if (existingUser) {
        return NextResponse.json({ error: 'รหัสพนักงาน (H.N.) นี้มีผู้ใช้งานแล้ว' }, { status: 400 });
      }
    }
    ...
    const userToReturn = {
      ...updatedUser,
      role: updatedUser.role === 'DEPT_HEAD' ? 'DEPARTMENT_HEAD' : updatedUser.role
    };
