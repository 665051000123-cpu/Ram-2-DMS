...
export default async function AuditLogsPage() {
  ...
  if (session.user.role !== 'SUPER_ADMIN' && session.user.role !== 'DEPARTMENT_HEAD') {
    return (
      <div className="p-6 text-center mt-20">
        ...
        <p className="text-slate-500 mt-2">คุณไม่มีสิทธิ์เข้าถึงหน้านี้ เฉพาะผู้ดูแลระบบ หรือหัวหน้าแผนกเท่านั้น</p>
      </div>
    );
  }
  ...
  const whereClause: any = session.user.role === 'SUPER_ADMIN' 
    ? {} 
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-slate-800">ประวัติการใช้งาน (Audit Logs)</h1>
